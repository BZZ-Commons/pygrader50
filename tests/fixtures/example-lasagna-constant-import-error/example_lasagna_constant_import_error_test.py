...
# Added for the pygrader50 corpus: upstream stops at the raise above, but our
# engine grades named test functions, and a real assignment declares some. They
# never run — the import fails first, which is exactly what this fixture checks.
class LasagnaTest(unittest.TestCase):
    def test_bake_time_remaining(self):
        self.assertEqual(bake_time_remaining(30), 10)

    def test_preparation_time_in_minutes(self):
        self.assertEqual(preparation_time_in_minutes(2), 4)
